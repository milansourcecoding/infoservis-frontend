...
export default function BuildingBankAccountDetailsPage() {
  const { t } = useLocales();
  ...
  const { isLoading, details } = useTypedSelector((state: RootState) => state.bankAccountSlice as any);
  ...
  const topSection = () => {
    ...
    >
      <Stack spacing={1} direction="row" alignItems="flex-start">
        <IconButton component={RouterLink} to={`/building/${building_id}`} state={{ activeTab: 'bank_accounts' }} >
          <Icon icon="eva:arrow-ios-back-fill" />
        </IconButton>

        <Stack spacing={0.5}>
          <Stack spacing={1} direction="row" alignItems="center">
            <Typography variant="h4">{details?.account_number}</Typography>
          </Stack>
          <Typography variant="body2" sx={{ color: 'text.disabled' }}>{t('bankAccount.details.account_number')}</Typography>
        </Stack>
      </Stack>
      ...
      <Stack direction="row" spacing={1}>
        ...
        >
          {t('bankAccount.details.back_to_list')}
        </Button>
      ...
      <Button
        ...
        onClick={onEditRow}
      >
        {t('bankAccount.details.edit_bank_account')}
      </Button>
      </Stack>
  ...
  const basicDataSection = () => {
    return <Card sx={{ p: 3, mb: 3 }}>
      ...
      <Grid container spacing={3}>
        {renderField(t('bankAccount.details.account_number'), details?.account_number)}
        {renderField(t('bankAccount.details.account_holder_name'), details?.account_holder_name)}
        {renderField(t('bankAccount.details.bank_name'), details?.bank_name)}
        {renderField(t('bankAccount.details.balance'), details?.balance)}
        {renderField(t('bankAccount.details.currency'), details?.currency)}
        {renderField(t('bankAccount.details.iban'), details?.iban)}
        {renderField(t('bankAccount.details.swift_code'), details?.swift_code)}
