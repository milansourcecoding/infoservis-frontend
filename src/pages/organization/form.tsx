...
import { getCities, escapeChars, getOrganizationTypes } from '../../utils/utils.tsx';
// import { OrganizationType } from '../../utils/enums.tsx';
...
export default function Form(props: any) {
  ...
  const cities = React.useMemo(() => getCities(), []);

  const organizationTypes = React.useMemo(() => getOrganizationTypes(), []);
    ...
    return <React.Fragment>
      ...
      <Grid item xs={12} md={6}>
        <FormControl fullWidth error={Boolean(errors.city)}>
          <DropdownAutocomplete
            ...
            options={cities}
            value={
              cities.find((city: any) => city.name === values.city) ?? null
            }
            onChange={(e, value) => {
              e.preventDefault();
              e.stopPropagation();
      ...
      <Grid item xs={12} md={6}>
        <FormControl fullWidth error={Boolean(errors.type)}>
          <DropdownAutocomplete
            ...
            disableClearable
            labelField="value"
            label={getFields(t, 'type').label}
            options={organizationTypes}
            value={organizationTypes.find((type: any) => type.id === values.type) ?? null}
            onChange={(e, value) => {
              e.preventDefault();
              e.stopPropagation();
      ...
      <Grid item xs={12}>
        <TextField
          ...
          error={Boolean(errors.responsible_person)}
          helperText={errors.responsible_person as string}
          onChange={handleChange}
          {...getFields(t, 'responsible_person')}
        />
      </Grid>
